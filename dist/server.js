...
import { TaprootUtils, constructClaimTransaction, } from 'boltz-core';
...
router.get('/payreq/:uuid', async (req, res) => {
...
router.post('/webhook/swap', async (req, res) => {
    ...
    try {
        ...
        const keys = ECPairFactory(ecc).fromPrivateKey(Buffer.from(swapData.privateKey, 'hex'));
        ...
        const tweakedKey = TaprootUtils.tweakMusig(musig, SwapTreeSerializer.deserializeSwapTree(swapData.swapTree).tree);
        ...
        const claimTx = targetFee(0.1, (fee) => constructClaimTransaction([{
                ...swapOutput,
                keys,
            ...
            }], address.toOutputScript(swapData.claimAddress, networks.liquid), fee, false), true);
        console.log(claimTx);
        // Get Boltz's partial signature
        const boltzSig = (await axios.post(`${process.env.BOLTZ_API_URL}/swap/reverse/${swapId}/claim`, {
            index: 0,
        ...
        musig.initializeSession(claimTx.hashForWitnessV1(0, [swapOutput.script], [Number(swapOutput.value)], Transaction.SIGHASH_DEFAULT, networks.liquid.genesisBlockHash));
        console.log(musig);
